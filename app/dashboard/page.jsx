  ...
  async function doCancel(booking, cancelSeries) {
    ...
    try {
      ...
      if (cancelSeries && booking.recurring_group_id) {
        ...
        setBalance(prev => prev + seriesBookings.length)
      } else {
    ...
    <main>
      {user && (
        <div style={{ marginBottom: '1.5rem' }}>
          ...
          {profile?.belt_rank && (
            <p style={{ color: '#cc0000', fontSize: '0.8rem', letterSpacing: '2px', textTransform: 'uppercase', margin: '0.25rem 0 0' }}>{profile.belt_rank} belt</p>
          )}
        </div>
      )}
      ...
      <div style={{ background: '#2a2a2a', border: '1px solid #cc0000', padding: '1rem 1.5rem', borderRadius: '8px', marginBottom: '2rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ color: '#666', fontSize: '0.85rem', textAlign: 'right' }}>
          Renew monthly<br /><span style={{ color: '#cc0000' }}>4 tokens/month</span>
        </div>
      </div>
          ...
          <div style={{ background: '#1a1a1a', border: '1px solid #cc0000', borderRadius: '12px', padding: '1.75rem', maxWidth: '380px', width: '100%' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              <button
                ...
                style={{ padding: '0.75rem', background: '#2a2a2a', color: '#fff', border: '1px solid #444', borderRadius: '8px', cursor: isProcessing ? 'not-allowed' : 'pointer', textAlign: 'left', opacity: isProcessing ? 0.6 : 1 }}
              >
              ...
              <button
                ...
                disabled={isProcessing}
                style={{ padding: '0.75rem', background: '#2a0000', color: '#fff', border: '1px solid #cc0000', borderRadius: '8px', cursor: isProcessing ? 'not-allowed' : 'pointer', textAlign: 'left', opacity: isProcessing ? 0.6 : 1 }}
              >
                <div style={{ fontWeight: 'bold', marginBottom: '2px' }}>All {cancelPrompt.seriesCount} lessons in this series</div>
                <div style={{ color: '#cc6666', fontSize: '0.8rem' }}>Cancels the entire recurring schedule — refund {cancelPrompt.seriesCount} tokens</div>
              </button>
              <button
              ...
              <div key={b.id} style={{ background: '#2a2a2a', border: '1px solid #333', borderRadius: '8px', padding: '1rem 1.5rem', marginBottom: '1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: '0.75rem' }}>
                  ...
                  <button
                    ...
                    disabled={isProcessing}
                    style={{ padding: '0.4rem 1rem', background: 'transparent', color: isProcessing ? '#555' : '#cc0000', border: `1px solid ${isProcessing ? '#555' : '#cc0000'}`, borderRadius: '4px', cursor: isProcessing ? 'not-allowed' : 'pointer', fontSize: '0.9rem' }}
                  >
              ...
              {series.map((b, i) => (
                <div key={b.id} style={{ background: i % 2 === 0 ? '#222' : '#1e1e1e', padding: '0.75rem 1.25rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: i < series.length - 1 ? '1px solid #2a2a2a' : 'none' }}>
                  ...
                  <button
                    onClick={() => handleCancelClick(b)}
                    disabled={isProcessing}
                    style={{ padding: '0.3rem 0.75rem', background: 'transparent', color: isProcessing ? '#555' : '#cc0000', border: `1px solid ${isProcessing ? '#555' : '#cc0000'}`, borderRadius: '4px', cursor: isProcessing ? 'not-allowed' : 'pointer', fontSize: '0.8rem' }}
                  >
                    {isProcessing ? '...' : 'Cancel'}
                  </button>
                </div>
